...
#############Defined Strings#############
addon_id     = 'plugin.video.mancavetv'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.mancavetv/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.mancavetv/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to Man Cave TV" 
			line3 = ('[COLOR blue]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('Man Cave TV', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("Man Cave TV", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search Man Cave TV:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('Man Cave TV', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('Man Cave TV', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			...
			pvrsetup()
			xbmcgui.Dialog().ok('Man Cave TV', 'PVR Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.mancavetv/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('Man Cave TV', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.mancavetv')
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('Man Cave TV','Would You like us to Setup the TV Guide for You?')
		if dialog:
				pvrsetup()
				xbmcgui.Dialog().ok('Man Cave TV', 'PVR Integration Complete')
